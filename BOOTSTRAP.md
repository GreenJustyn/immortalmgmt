...
5. 🐧 **WSL (Windows Subsystem for Linux)**: Recommended for advanced cross-platform media replication features. *(Note: If executing custom pipelines like `01 - MediaManager.ps1`, the framework will attempt to automatically provision and configure WSL, Ubuntu, and all Linux prerequisites dynamically on first run).*
