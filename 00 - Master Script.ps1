...
foreach ($job in $syncJobsLocalToPlex) {
    ...
    # Added --itemize-changes for detailed logging and --timeout to prevent network hangs
    $wslCommand = "sshpass -f $wslCredFilePath rsync -avz --itemize-changes --timeout=60 --remove-source-files -e 'ssh -o StrictHostKeyChecking=no' '$($job.WslSource)' '$($job.Dest)'"
    
    try {
        # Safe Output Handling: Write directly to temp files to avoid .NET buffer deadlocks
...
foreach ($job in $syncJobsPlexToJellyfin) {
    ...
    # Inner command requires SSH configuration explicitly passed to bypass prompts
    # Added --itemize-changes for granular tracking and --timeout=60
    ...
    try {
        # Safe Output Handling: Write directly to temp files
        $tempOut = New-TemporaryFile
        $tempErr = New-TemporaryFile
