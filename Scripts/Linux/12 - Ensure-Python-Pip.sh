...
PROVIDER_NAME=$(python3 -c "import json, sys; print(json.loads(sys.argv[1]).get('ProviderName', ''))" "$CONFIG_JSON")
MIN_VERSION=$(python3 -c "import json, sys; print(json.loads(sys.argv[1]).get('MinimumVersion', ''))" "$CONFIG_JSON")

write_log "Loaded Configuration Variables:" "INFO"
write_log "  ProviderName = $PROVIDER_NAME" "INFO"
write_log "  MinimumVersion = $MIN_VERSION" "INFO"

# Check if running as root
if [ "$EUID" -ne 0 ]; then
    write_log "This script must be run as root to install package providers." "CRITICAL"
    exit 1
fi

# Scenario 1: PowerShell on Linux is available
if command -v pwsh &> /dev/null; then
    write_log "PowerShell (pwsh) detected. Attempting to install NuGet provider for pwsh." "INFO"
    
    pwsh -Command "
    \$Provider = Get-PackageProvider -Name '$PROVIDER_NAME' -ListAvailable -ErrorAction SilentlyContinue | Where-Object { \$_.Version -ge [version]'$MIN_VERSION' }
    if (-not \$Provider) {
        Write-Host 'Installing NuGet provider...'
        Install-PackageProvider -Name '$PROVIDER_NAME' -MinimumVersion '$MIN_VERSION' -Force -Scope AllUsers -ErrorAction Stop
    } else {
    ...
    if [ $? -eq 0 ]; then
        write_log "NuGet provider check/install in pwsh completed." "INFO"
    else
        write_log "Failed to install NuGet provider in pwsh." "ERROR"
    fi
else
    write_log "PowerShell (pwsh) not detected. Checking for Python pip as Linux management package provider." "INFO"
    
    if command -v pip3 &> /dev/null; then
        write_log "pip3 is available." "INFO"
    else
        write_log "pip3 not found. Python packages cannot be managed easily. Please install python3-pip." "WARNING"
    fi
fi
