...
write_log "Initializing SMART Telemetry logging..." "INFO"

# Admin Check (Required for smartctl on device nodes)
if [ "$EUID" -ne 0 ]; then
    write_log "⚠️ CRITICAL: Elevated privilege execution required to communicate directly with SMART controllers. Please run as root." "CRITICAL"
    exit 1
fi

# Run Python helper
write_log "Running Python helper to dump SMART telemetry..." "INFO"
...
write_log("Verifying \"smartctl\" package status...", "INFO")
...
try:
    ...
    for line in scan_out.splitlines():
        ...
        write_log(f"Querying target: {dev}...", "INFO")
        
        now = datetime.datetime.now()
        filename = os.path.join(output_dir, f"SMART-dev{dev_no}-{now.year}-{now.month}-{now.day}.json")
        write_log(f"Offloading to file: {filename}...", "INFO")
    ...
    write_log("✅ S.M.A.R.T payload dumps captured.", "INFO")
except Exception as e:
    write_log(f"Failed to scan devices: {e}", "CRITICAL")
    sys.exit(1)
' "$LOG_FILE" "$ENVIRONMENT" "$OUTPUT_DIR"

if [ $? -eq 0 ]; then
    write_log "SMART query completed successfully." "INFO"
else
    write_log "SMART query failed." "ERROR"
fi
