...
write_log "Initializing physical memory checks..." "INFO"

# Run Python helper
write_log "Running Python helper to gather RAM info..." "INFO"
...
if os.geteuid() == 0:
    try:
        output = subprocess.check_output(["dmidecode", "-t", "memory"]).decode()
        ...
        for dev in devices[1:]:
            ...
            for line in lines:
                if "Size:" in line:
                    ...
                    locator = line.split(":")[1].strip()
                elif "Type:" in line:
                    type_str = line.split(":")[1].strip()
            
            if size and size != "No Module Installed":
                write_log(f"✅ {size} {type_str} at {speed} in {locator} by {manufacturer}", "INFO")
                found = True
                
        if not found:
             write_log("No physical RAM modules found via dmidecode (could be a VM).", "INFO")
    except Exception as e:
        write_log(f"Failed to run dmidecode: {e}", "WARNING")
else:
    write_log("Running as non-root. Falling back to /proc/meminfo for total memory.", "INFO")

# Fallback to total memory from /proc/meminfo
try:
    with open("/proc/meminfo", "r") as f:
        for line in f:
            if line.startswith("MemTotal:"):
                total_kb = int(line.split()[1])
                total_str = bytes_2_string(total_kb * 1024)
                write_log(f"Total System Memory: {total_str}", "INFO")
                break
except Exception as e:
    write_log(f"Failed to read /proc/meminfo: {e}", "WARNING")

' "$LOG_FILE" "$ENVIRONMENT"

if [ $? -eq 0 ]; then
    write_log "Memory check completed successfully." "INFO"
else
    write_log "Memory check failed." "ERROR"
fi
