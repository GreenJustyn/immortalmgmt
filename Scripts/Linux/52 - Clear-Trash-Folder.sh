...
write_log "Initializing absolute Recycle Bin deletion sequence..." "INFO"

# Core Logic
TRASH_DIR="$HOME/.local/share/Trash/files"

if [ -d "$TRASH_DIR" ]; then
    write_log "Found Trash directory at $TRASH_DIR. Clearing..." "INFO"
    # Remove contents, not the directory
    # Use find to avoid "Argument list too long" if there are many files
    find "$TRASH_DIR" -mindepth 1 -delete 2>/dev/null
    write_log "✅ Global trash bin partitions cleared cleanly." "INFO"
else
    write_log "No Trash directory found at $TRASH_DIR. Conceptual Recycle Bin is empty." "INFO"
fi

write_log "Maintenance cycle complete." "INFO"
post_flight
