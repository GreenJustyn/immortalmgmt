...
write_log "Initializing script execution: MAC Header Pattern Validations." "INFO"

if [ -z "$MAC_ADDRESS" ]; then
    write_log "Empty MAC provided for validation parameters." "WARNING"
    exit 0
fi

write_log "Validating MAC address: $MAC_ADDRESS" "INFO"
...
if re.match(regex, mac):
    write_log(f"✅ MAC address {mac} is verified valid under pattern constraints.", "INFO")
    sys.exit(0)
else:
    write_log(f"Invalid MAC format parsed: [{mac}]", "ERROR")
    sys.exit(1)
' "$MAC_ADDRESS" "$LOG_FILE" "$ENVIRONMENT"

if [ $? -eq 0 ]; then
    write_log "MAC check completed successfully." "INFO"
else
    write_log "MAC check failed." "ERROR"
fi
