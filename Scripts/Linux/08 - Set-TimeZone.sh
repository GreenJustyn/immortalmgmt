...
WINDOWS_TZ=$(python3 -c '
import json
import sys
try:
    with open(sys.argv[1], "r") as f:
        config = json.load(f)
    print(config.get("TimeZoneId", ""))
except Exception as e:
    print("")
' "$CONFIG_FILE")

if [ -z "$WINDOWS_TZ" ]; then
    write_log "FATAL: TimeZoneId not found in config." "CRITICAL"
    exit 1
fi

write_log "Loaded Configuration Variables:" "INFO"
write_log "  TimeZoneId (Windows) = $WINDOWS_TZ" "INFO"

# Mapping Windows to Linux Timezone
LINUX_TZ="$WINDOWS_TZ"
if [ "$WINDOWS_TZ" == "AUS Eastern Standard Time" ]; then
    LINUX_TZ="Australia/Sydney"
    write_log "Mapped Windows TimeZone '$WINDOWS_TZ' to Linux '$LINUX_TZ'" "INFO"
fi

# Check if running as root
if [ "$EUID" -ne 0 ]; then
    write_log "This script must be run as root to change timezone." "CRITICAL"
    exit 1
fi
...
if [ "$CURRENT_TZ" != "$LINUX_TZ" ]; then
    write_log "TimeZone mismatch. Current: $CURRENT_TZ. Enforcing: $LINUX_TZ" "INFO"
    
    # Verify timezone is valid on Linux
    if ! timedatectl list-timezones | grep -qF "$LINUX_TZ"; then
        write_log "Invalid Linux TimeZone: $LINUX_TZ. Cannot set." "ERROR"
        exit 1
    fi
    
    timedatectl set-timezone "$LINUX_TZ"
    if [ $? -eq 0 ]; then
        write_log "TimeZone successfully updated to $LINUX_TZ." "INFO"
    else
        write_log "Failed to update TimeZone." "ERROR"
        exit 1
    fi
else
    write_log "TimeZone is already set to $LINUX_TZ. No action taken." "INFO"
fi
