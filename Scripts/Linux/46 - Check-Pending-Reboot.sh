...
write_log "Initializing pending reboot evaluations..." "INFO"
...
if command -v needs-restarting &> /dev/null; then
    if ! needs-restarting -r &> /dev/null; then
        PENDING=1
        REASON="${REASON:+$REASON, }needs-restarting indicated reboot required"
    fi
fi

if [ "$PENDING" -eq 1 ]; then
    write_log "⚠️ Pending reboot ($REASON)" "WARNING"
else
    write_log "✅ No pending reboot." "INFO"
fi

write_log "Reboot check completed." "INFO"
post_flight
