...
MANIFEST_PATH=$(python3 -c "import json, sys; print(json.loads(sys.argv[1]).get('ManifestPath', ''))" "$CONFIG_JSON")

# Translate path if it looks like Windows
if [[ "$MANIFEST_PATH" == *":"* ]]; then
    MANIFEST_PATH="$BASE_DIR/Variables/_AutomationManifest.json"
    write_log "Translated Windows path to Linux: $MANIFEST_PATH" "INFO"
fi

write_log "Loaded Configuration Variables:" "INFO"
write_log "  ManifestPath = $MANIFEST_PATH" "INFO"

write_log "Generating Infrastructure Manifest..." "INFO"
...
manifest = {
    ...
    "LastRunTime": datetime.datetime.now().strftime("%Y-%m-%d %H:%M:%S"),
    "FrameworkVersion": config.get("Version", "1.0.0"),
    "PowerShellVer": "N/A (Linux Bash)",
...
try:
    ...
    print(f"INFO: Manifest successfully written to {manifest_path}.")
except Exception as e:
    print(f"ERROR: Failed to write manifest: {e}")
    sys.exit(1)
' "$CONFIG_JSON" "$MANIFEST_PATH" "$ENVIRONMENT"

if [ $? -eq 0 ]; then
    write_log "Manifest generated successfully." "INFO"
else
    write_log "Failed to generate manifest." "ERROR"
fi
