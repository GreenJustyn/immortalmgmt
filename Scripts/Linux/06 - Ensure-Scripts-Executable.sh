...
write_log "Note: On Linux, Execution Policy translates to file execute permissions (chmod +x)." "INFO"

# Load Config (optional for this implementation, but let's read it to be consistent)
...
# Find all .sh scripts in the Linux folder
scripts=$(find "$SCRIPT_DIR" -name "*.sh" -type f)

write_log "Checking execute permissions for scripts in $SCRIPT_DIR..." "INFO"

changed=0
for script in $scripts; do
    if [ ! -x "$script" ]; then
        write_log "Script $script is not executable. Granting execute permission." "INFO"
        chmod +x "$script"
        changed=$((changed + 1))
    fi
done
...
if [ $changed -gt 0 ]; then
    write_log "Granted execute permissions to $changed scripts." "INFO"
else
    write_log "All scripts already have execute permissions. No action taken." "INFO"
fi
