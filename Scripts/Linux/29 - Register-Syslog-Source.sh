...
write_log "Note: On Linux, custom log sources are not registered. You simply use a tag with the logger command." "INFO"
...
SOURCE_NAME=$(python3 -c "import json, sys; print(json.loads(sys.argv[1]).get('SourceName', 'MasterRunner'))" "$CONFIG_JSON")

write_log "Loaded Configuration Variables:" "INFO"
write_log "  SourceName = $SOURCE_NAME" "INFO"

write_log "Verifying custom source by writing a test message to system log..." "INFO"

# Use logger to write a message with the specific tag
logger -t "$SOURCE_NAME" "Custom EventLog Source '$SOURCE_NAME' verified on Linux."

if [ $? -eq 0 ]; then
    write_log "Successfully wrote test message with tag '$SOURCE_NAME'." "INFO"
    write_log "You can view it using: journalctl -t $SOURCE_NAME" "INFO"
else
    write_log "Failed to write message using logger." "ERROR"
fi
