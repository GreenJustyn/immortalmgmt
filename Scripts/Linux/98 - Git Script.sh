...
# Admin Check
if [ "$EUID" -ne 0 ]; then
    write_log "⚠️ CRITICAL: Package installation requires root privileges." "CRITICAL"
    exit 1
fi

# Core Logic
if command -v git &> /dev/null; then
    write_log "✅ Git is already installed." "INFO"
    git --version >> "$LOG_FILE"
else
    write_log "Git not found. Attempting to install..." "INFO"
    if command -v apt-get &> /dev/null; then
        write_log "Using apt-get..." "INFO"
        apt-get update -y >> "$LOG_FILE" 2>&1
        apt-get install -y git >> "$LOG_FILE" 2>&1
    elif command -v dnf &> /dev/null; then
        write_log "Using dnf..." "INFO"
        dnf install -y git >> "$LOG_FILE" 2>&1
    elif command -v yum &> /dev/null; then
        write_log "Using yum..." "INFO"
        yum install -y git >> "$LOG_FILE" 2>&1
    else
        write_log "No supported package manager found (apt, dnf, yum)." "CRITICAL"
        exit 1
    fi
    
    if command -v git &> /dev/null; then
        write_log "✅ Git installed successfully." "INFO"
        git --version >> "$LOG_FILE"
    else
        write_log "Failed to install Git." "CRITICAL"
        exit 1
    fi
fi
