...
SHOW_RECENT=$(python3 -c "import json, sys; print(json.loads(sys.argv[1]).get('ShowRecentFiles', 0))" "$CONFIG_JSON")

write_log "Loaded Configuration Variables:" "INFO"
write_log "  ShowRecentFiles = $SHOW_RECENT" "INFO"

# GNOME Quick Access Equivalent (Recent Files)
if command -v gsettings &> /dev/null; then
    write_log "gsettings detected. Enforcing GNOME privacy settings." "INFO"
    
    # Warning if running as root
    if [ "$EUID" -eq 0 ]; then
        write_log "WARNING: Running as root. gsettings changes may not apply to the logged-in user session." "WARNING"
    fi
    
    # Map 0 to false, non-zero to true
    if [ "$SHOW_RECENT" -eq 0 ]; then
        VAL="false"
    else
        VAL="true"
    fi
    
    CURRENT_VAL=$(gsettings get org.gnome.desktop.privacy remember-recent-files)
    
    if [ "$CURRENT_VAL" != "$VAL" ]; then
        write_log "Setting remember-recent-files to $VAL." "INFO"
        gsettings set org.gnome.desktop.privacy remember-recent-files "$VAL"
        
        if [ $? -eq 0 ]; then
            write_log "GNOME privacy settings updated successfully." "INFO"
        else
            write_log "Failed to update GNOME privacy settings." "ERROR"
        fi
    else
        write_log "GNOME privacy settings are already correct ($VAL). No action taken." "INFO"
    fi
else
    write_log "gsettings not found. This script currently only supports GNOME desktop environment settings." "WARNING"
    write_log "If you are using a headless server or different desktop, this setting is likely irrelevant or managed differently." "INFO"
fi
