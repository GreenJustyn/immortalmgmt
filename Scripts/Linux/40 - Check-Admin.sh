...
if errors:
    ...
    body = f"Privilege Alert: Non-Admin State detected in {script_name} run:\n\n" + "".join(errors)
    msg = MIMEText(body)
...
write_log "Initializing script execution: Admin Privilege Check." "INFO"

# Core Privilege Validation
CURRENT_USER=$(whoami)

if [ "$EUID" -eq 0 ]; then
    write_log "✅ Yes, $CURRENT_USER has root (admin) rights." "INFO"
else
    write_log "⚠️ No, $CURRENT_USER has normal user rights only. Root privileges required for full admin tasks." "ERROR"
fi

write_log "Privilege check finalized." "INFO"
post_flight
