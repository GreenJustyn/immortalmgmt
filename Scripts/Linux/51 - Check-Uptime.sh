...
write_log "Initializing duration timing logic..." "INFO"

# Run Python helper
write_log "Running Python helper to gather uptime info..." "INFO"
...
def timespan_as_string(seconds):
...
try:
    ...
    try:
        result = subprocess.run(["needs-restarting", "-r"], capture_output=True)
        if result.returncode != 0:
             status = "WARNING"
             pending = "with pending reboot (needs-restarting indicated reboot required)"
    except:
        pass
        
    hostname = os.uname()[1]
    
    uptime_str = timespan_as_string(uptime_seconds)
    boot_str = boot_time.strftime("%Y-%m-%d")
    
    write_log(f"{hostname} is up for {uptime_str} since {boot_str} {pending}", status)
    
except Exception as e:
    write_log(f"Failed to query uptime: {e}", "CRITICAL")
    sys.exit(1)
' "$LOG_FILE" "$ENVIRONMENT"

if [ $? -eq 0 ]; then
    write_log "Uptime check completed successfully." "INFO"
else
    write_log "Uptime check failed." "ERROR"
fi
