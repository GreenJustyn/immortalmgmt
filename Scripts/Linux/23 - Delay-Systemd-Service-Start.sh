...
write_log "Note: On Linux, 'Delayed Start' is implemented via systemd drop-in files with sleep." "INFO"
...
SERVICES=$(python3 -c "import json, sys; print(' '.join(json.loads(sys.argv[1]).get('ServicesToDelay', [])))" "$CONFIG_JSON")

write_log "Loaded Configuration Variables:" "INFO"
write_log "  ServicesToDelay = $SERVICES" "INFO"

# Check if running as root
if [ "$EUID" -ne 0 ]; then
    write_log "This script must be run as root to configure services." "CRITICAL"
    exit 1
fi

for svc in $SERVICES; do
    write_log "Processing service: $svc" "INFO"
    
    # Check if service exists
    if systemctl list-unit-files "$svc.service" &> /dev/null; then
        write_log "Service '$svc' exists. Configuring delayed start." "INFO"
        
        DROPIN_DIR="/etc/systemd/system/$svc.service.d"
        DROPIN_FILE="$DROPIN_DIR/delay.conf"
        
        # Check if already configured
        if [ -f "$DROPIN_FILE" ] && grep -q "ExecStartPre=/bin/sleep" "$DROPIN_FILE"; then
            write_log "Service '$svc' is already configured for delayed start." "INFO"
        else
            write_log "Creating drop-in file to delay service start by 30 seconds." "INFO"
            mkdir -p "$DROPIN_DIR"
            cat > "$DROPIN_FILE" <<EOF
[Service]
...
EOF
            ...
            write_log "Reloading systemd daemon." "INFO"
            systemctl daemon-reload
            write_log "Service '$svc' configured successfully." "INFO"
        fi
    else
        write_log "Service '$svc' does not exist on this system. Skipping." "WARNING"
    fi
done
