...
write_log "Note: On Linux, search indexing is handled by updatedb (locate) or desktop indexers like Tracker." "INFO"
...
if [ "$EUID" -ne 0 ]; then
    write_log "This script must be run as root to disable system-wide indexing timers." "CRITICAL"
    exit 1
fi

# 1. Disable plocate/mlocate updatedb timer (CLI search indexer)
# This is the most common background indexer on servers.
TIMERS=("plocate-updatedb.timer" "mlocate-updatedb.timer")
TIMER_DISABLED=false

for timer in "${TIMERS[@]}"; do
    if systemctl list-unit-files "$timer" &> /dev/null; then
        write_log "Found search indexing timer: $timer" "INFO"
        
        if systemctl is-active --quiet "$timer" || [ "$(systemctl is-enabled "$timer" 2>/dev/null)" == "enabled" ]; then
            write_log "Stopping and disabling $timer to save IOPS." "INFO"
            systemctl stop "$timer" 2>/dev/null
            systemctl disable "$timer" 2>/dev/null
            systemctl mask "$timer" 2>/dev/null
            write_log "$timer disabled and masked successfully." "INFO"
            TIMER_DISABLED=true
        else
            write_log "$timer is already inactive and disabled." "INFO"
        fi
    fi
done

if [ "$TIMER_DISABLED" = false ]; then
    write_log "No active mlocate or plocate timers found. Skipping." "INFO"
fi

# 2. Warn about Tracker (GNOME desktop indexer)
# Tracker runs in the user session, so root cannot easily disable it system-wide.
if command -v tracker3 &> /dev/null || command -v tracker &> /dev/null; then
    write_log "GNOME Tracker indexer detected. Tracker runs in the user session." "WARNING"
    write_log "To disable Tracker for a user, run: tracker3 daemon -s OR configure it via gsettings as the user." "INFO"
fi
