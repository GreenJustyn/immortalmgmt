...
write_log "Note: On Linux, wallpaper management is specific to the Desktop Environment and usually applies per-user." "INFO"
...
if [ "$EUID" -eq 0 ]; then
    write_log "Running as root. Wallpaper management via gsettings requires a user session context." "WARNING"
    write_log "Skipping execution. To disable wallpaper on GNOME, run this script as the target user." "INFO"
    
    # Note: The active code for GNOME desktop would look like this if run as user:
    #
    # if command -v gsettings &> /dev/null; then
    #     gsettings set org.gnome.desktop.background picture-uri ""
    #     gsettings set org.gnome.desktop.background picture-uri-dark ""
    #     write_log "Wallpaper disabled for current user in GNOME." "INFO"
    # fi
else
    # If running as user, try to use gsettings
    if command -v gsettings &> /dev/null; then
        write_log "gsettings detected. Attempting to disable wallpaper for current user." "INFO"
        ...
        write_log "gsettings not found. Cannot configure wallpaper." "WARNING"
    fi
fi
