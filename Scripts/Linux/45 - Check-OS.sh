...
write_log "Initializing Operating System status audit..." "INFO"

# Run Python helper
write_log "Running Python helper to gather OS info..." "INFO"
...
try:
    if hasattr(platform, "freedesktop_os_release"):
        ...
        os_version = release_info.get("VERSION", "")
    else:
...
write_log(f"✅ {os_name} {os_version} (Linux kernel {kernel} on {arch})", "INFO")
' "$LOG_FILE" "$ENVIRONMENT"

if [ $? -eq 0 ]; then
    write_log "OS check completed successfully." "INFO"
else
    write_log "OS check failed." "ERROR"
fi
