...
IP_ADDRESS=$(python3 -c "import json, sys; print(json.loads(sys.argv[1]).get('IPAddress', ''))" "$CONFIG_JSON")
PREFIX_LENGTH=$(python3 -c "import json, sys; print(json.loads(sys.argv[1]).get('PrefixLength', ''))" "$CONFIG_JSON")
DEFAULT_GATEWAY=$(python3 -c "import json, sys; print(json.loads(sys.argv[1]).get('DefaultGateway', ''))" "$CONFIG_JSON")

if [ -z "$INTERFACE_ALIAS" ] || [ -z "$IP_ADDRESS" ]; then
    write_log "FATAL: Required config missing (InterfaceAlias or IPAddress)." "CRITICAL"
    exit 1
fi

write_log "Loaded Configuration Variables:" "INFO"
write_log "  InterfaceAlias = $INTERFACE_ALIAS" "INFO"
write_log "  IPAddress = $IP_ADDRESS" "INFO"
write_log "  PrefixLength = $PREFIX_LENGTH" "INFO"
write_log "  DefaultGateway = $DEFAULT_GATEWAY" "INFO"

# Check if running as root
if [ "$EUID" -ne 0 ]; then
    write_log "This script must be run as root to change network settings." "CRITICAL"
    exit 1
fi
...
# Idempotent Logic
CURRENT_IP=$(ip -4 addr show dev "$INTERFACE_ALIAS" | grep -oP '(?<=inet\s)\d+(\.\d+){3}' | head -n 1)
CURRENT_GW=$(ip route show dev "$INTERFACE_ALIAS" | grep default | awk '{print $3}' | head -n 1)

if [ "$CURRENT_IP" != "$IP_ADDRESS" ] || [ "$CURRENT_GW" != "$DEFAULT_GATEWAY" ]; then
    write_log "Network configuration drift detected. Enforcing static IP: $IP_ADDRESS" "INFO"
    
    # Try nmcli first for persistence
    if command -v nmcli &> /dev/null; then
        write_log "Using nmcli to set static IP." "INFO"
        CON_NAME=$(nmcli -t -f NAME,DEVICE connection show --active | grep ":$INTERFACE_ALIAS$" | cut -d: -f1)
        if [ -z "$CON_NAME" ]; then
            CON_NAME="$INTERFACE_ALIAS"
        fi
        
        nmcli con mod "$CON_NAME" ipv4.addresses "$IP_ADDRESS/$PREFIX_LENGTH" ipv4.gateway "$DEFAULT_GATEWAY" ipv4.method manual
        nmcli con up "$CON_NAME"
    else
        write_log "nmcli not found. Using ip commands (non-persistent across reboots)." "WARNING"
        ip addr flush dev "$INTERFACE_ALIAS"
        ip addr add "$IP_ADDRESS/$PREFIX_LENGTH" dev "$INTERFACE_ALIAS"
        ip route add default via "$DEFAULT_GATEWAY" dev "$INTERFACE_ALIAS"
    fi
    
    write_log "Static IP and Gateway enforced successfully." "INFO"
else
    write_log "IP configuration is already correct ($IP_ADDRESS). No action taken." "INFO"
fi
