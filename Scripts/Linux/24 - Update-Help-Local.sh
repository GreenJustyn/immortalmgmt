...
FORCE=$(python3 -c "import json, sys; print(json.loads(sys.argv[1]).get('Force', True))" "$CONFIG_JSON")

write_log "Loaded Configuration Variables:" "INFO"
write_log "  Force = $FORCE" "INFO"

# Check if pwsh exists
if command -v pwsh &> /dev/null; then
    write_log "PowerShell (pwsh) detected. Updating help files." "INFO"
    ...
    # Update help
    pwsh -Command "Update-Help $FORCE_ARG -ErrorAction SilentlyContinue"
    
    if [ $? -eq 0 ]; then
        write_log "PowerShell help files updated successfully." "INFO"
    else
        write_log "PowerShell help update encountered errors (normal for some modules)." "WARNING"
    fi
else
    write_log "PowerShell (pwsh) not detected. Skipping help update." "INFO"
    write_log "Note: This script is specific to PowerShell help files." "INFO"
fi
