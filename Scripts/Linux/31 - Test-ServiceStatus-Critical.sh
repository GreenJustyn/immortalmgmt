...
if errors:
    ...
    body = f"The following service failures or script errors were detected in the {script_name} run:\n\n" + "".join(errors)
    msg = MIMEText(body)
...
CRITICAL_SERVICES=$(python3 -c "import json, sys; print(' '.join(json.loads(sys.argv[1]).get('CriticalServices', [])))" "$CONFIG_JSON")

write_log "Loaded Configuration Variables:" "INFO"
write_log "  CriticalServices = $CRITICAL_SERVICES" "INFO"

# Check if running as root
if [ "$EUID" -ne 0 ]; then
    write_log "This script must be run as root to manage services." "CRITICAL"
    exit 1
fi

for svc in $CRITICAL_SERVICES; do
    # Map Windows service names to Linux equivalents
    target_svc="$svc"
    if [ "$svc" == "WinRM" ]; then
        target_svc="ssh"
        write_log "Mapping Windows service 'WinRM' to Linux 'ssh'." "INFO"
    elif [ "$svc" == "LanmanServer" ]; then
        target_svc="smbd"
        write_log "Mapping Windows service 'LanmanServer' to Linux 'smbd'." "INFO"
    fi
    
    if systemctl list-unit-files "$target_svc.service" &> /dev/null; then
        if systemctl is-active --quiet "$target_svc"; then
            write_log "Service '$target_svc' is running normally." "INFO"
        else
            write_log "Service '$target_svc' is not running. Attempting recovery..." "CRITICAL"
            
            systemctl start "$target_svc"
            
            if [ $? -eq 0 ]; then
                write_log "Successfully restarted service '$target_svc'." "INFO"
            else
                write_log "Failed to restart service '$target_svc'. Requires manual intervention." "ERROR"
            fi
        fi
    else
        write_log "Service '$target_svc' not found on this system." "WARNING"
    fi
done
