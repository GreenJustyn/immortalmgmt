...
try {
    ...
    $DriftDetected = $false
    $Smb1State = $SmbConfig.EnableSMB1Protocol
    $SigState = $SmbConfig.RequireSecuritySignature

    if ($Smb1State -ne $Config.EnableSMB1Protocol) {
        Write-Log "SMBv1 is currently set to $Smb1State. Enforcing: $($Config.EnableSMB1Protocol)" "INFO"
        Set-SmbServerConfiguration -EnableSMB1Protocol $Config.EnableSMB1Protocol -Force -ErrorAction Stop
        $DriftDetected = $true
    }

    if ($SigState -ne $Config.RequireSecuritySignature) {
        Write-Log "SMB Security Signature is currently set to $SigState. Enforcing: $($Config.RequireSecuritySignature)" "INFO"
        Set-SmbServerConfiguration -RequireSecuritySignature $Config.RequireSecuritySignature -Force -ErrorAction Stop
        $DriftDetected = $true
    }

    if (-not $DriftDetected) {
        Write-Log "SMB Server Configuration is already secure and matches desired state. No action taken." "INFO"
    } else {
        Write-Log "SMB Server Configuration updated successfully." "INFO"
    }
