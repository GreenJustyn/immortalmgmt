    ...
    try {
        # Credential Check (included if your downstream logic requires it)
            ...
            if (-not (Test-Path $Config.QuotaPath)) {
                Write-Log "Quota path $($Config.QuotaPath) does not exist. Creating directory." "INFO"
                New-Item -Path $Config.QuotaPath -ItemType Directory -Force -ErrorAction Stop | Out-Null
            }

            $LimitBytes = $Config.SizeLimitGB * 1GB
            $ExistingQuota = Get-FsrmQuota -Path $Config.QuotaPath -ErrorAction SilentlyContinue

            if (-not $ExistingQuota) {
                Write-Log "No quota found on $($Config.QuotaPath). Creating a new $($Config.SizeLimitGB)GB hard quota." "INFO"
                New-FsrmQuota -Path $Config.QuotaPath -Size $LimitBytes -ErrorAction Stop | Out-Null
                Write-Log "Directory quota created successfully." "INFO"
            } elseif ($ExistingQuota.Size -ne $LimitBytes) {
                Write-Log "Quota mismatch on $($Config.QuotaPath). Updating size to $($Config.SizeLimitGB)GB." "INFO"
                Set-FsrmQuota -Path $Config.QuotaPath -Size $LimitBytes -ErrorAction Stop | Out-Null
                Write-Log "Directory quota updated successfully." "INFO"
            } else {
                Write-Log "Quota on $($Config.QuotaPath) is already correctly set to $($Config.SizeLimitGB)GB. No action taken." "INFO"
            }
    } catch {
