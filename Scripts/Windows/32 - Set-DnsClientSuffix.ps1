...
try {
    ...
    if (-not $Interface) { 
        throw "Interface $($Config.InterfaceAlias) not found." 
    }

    # Idempotent Logic
    $CurrentSuffix = (Get-DnsClient -InterfaceAlias $Config.InterfaceAlias).ConnectionSpecificSuffix

    if ($CurrentSuffix -ne $Config.DnsSuffix) {
        Write-Log "DNS Suffix drift detected. Enforcing: $($Config.DnsSuffix)" "INFO"
        
        Set-DnsClient -InterfaceAlias $Config.InterfaceAlias -ConnectionSpecificSuffix $Config.DnsSuffix -ErrorAction Stop
        
        Write-Log "DNS Suffix updated successfully." "INFO"
    } else {
        Write-Log "DNS Suffix is already correct ($($Config.DnsSuffix)). No action taken." "INFO"
    }
