...
try {
    ...
    try {
        # Idempotent Logic
            ...
            $CurrentRecent = (Get-ItemProperty -Path $RegPath -Name "ShowRecent" -ErrorAction SilentlyContinue).ShowRecent
            $CurrentFrequent = (Get-ItemProperty -Path $RegPath -Name "ShowFrequent" -ErrorAction SilentlyContinue).ShowFrequent

            if (($CurrentRecent -ne $Config.ShowRecentFiles) -or ($CurrentFrequent -ne $Config.ShowFrequentFolders)) {
                Write-Log "Quick Access settings drift detected. Enforcing standard admin view." "INFO"

                Set-ItemProperty -Path $RegPath -Name "ShowRecent" -Value $Config.ShowRecentFiles -Force -ErrorAction Stop
                Set-ItemProperty -Path $RegPath -Name "ShowFrequent" -Value $Config.ShowFrequentFolders -Force -ErrorAction Stop

                Write-Log "Explorer Quick Access telemetry disabled." "INFO"
            } else {
                Write-Log "Explorer Quick Access settings are already correct. No action taken." "INFO"
            }
    } catch {
