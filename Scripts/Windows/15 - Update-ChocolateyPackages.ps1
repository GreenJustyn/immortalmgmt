    ...
    try {
        # Credential Check
            ...
            if (-not (Test-Path $ChocoExe)) {
                throw "FATAL: Chocolatey is not installed. Skipping package updates."
            }

            Write-Log "Executing Chocolatey upgrade process." "INFO"

            if ($Config.UpgradeAll) {
                # Run silent upgrade on all packages
                $Output = & $ChocoExe upgrade all -y 2>&1

                # Check native exit code (0 = Success, 2 = No updates found)
                if ($LASTEXITCODE -notin @(0, 2)) {
                    Write-Log "Chocolatey upgrade failed with code $LASTEXITCODE. Output: $($Output -join ' ')" "ERROR"
                } else {
                    Write-Log "Chocolatey output: $($Output -join ' ')" "INFO"
                }
            } else {
                foreach ($Pkg in $Config.SpecificPackages) {
                    $Output = & $ChocoExe upgrade $Pkg -y 2>&1
            ...
            Write-Log "Chocolatey update process completed." "INFO"
    } catch {
