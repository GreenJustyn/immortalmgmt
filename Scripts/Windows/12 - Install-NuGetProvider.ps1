...
try {
    ...
    try {
        # Credential Check (included if your downstream logic requires it)
            ...
            # Idempotent Logic
            $Provider = Get-PackageProvider -Name $Config.ProviderName -ListAvailable -ErrorAction SilentlyContinue | Where-Object { $_.Version -ge [version]$Config.MinimumVersion }

            if (-not $Provider) {
                Write-Log "NuGet provider missing or outdated. Installing now." "INFO"

                # ErrorAction Stop ensures failures trigger the catch block and email alert
                Install-PackageProvider -Name $Config.ProviderName -MinimumVersion $Config.MinimumVersion -Force -Scope AllUsers -ErrorAction Stop

                Write-Log "NuGet provider successfully installed." "INFO"
            } else {
                Write-Log "NuGet provider (Version $($Provider[0].Version)) is already installed. No action taken." "INFO"
            }
    } catch {
