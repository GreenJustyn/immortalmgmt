...
try {
    ...
    try {
        # Operational Logic
            ...
            try {
                # Attempt to write the manifest
                $ManifestData | ConvertTo-Json | Out-File -FilePath $Config.ManifestPath -Force -ErrorAction Stop
                Write-Log "Manifest successfully written to $($Config.ManifestPath)." "INFO"
            } catch {
                # If writing fails, throw an error to trigger the catch block and email alert
                throw "Failed to write JSON manifest. $($_.Exception.Message)"
            }
    } catch {
