    ...
    try {
        # Credential Check (included if your downstream logic requires it)
            ...
            if (-not (Test-Path $WerRegPath)) {
                Write-Log "Registry path '$WerRegPath' not found. Creating it..." "INFO"
                New-Item -Path $WerRegPath -Force | Out-Null
            }

            $CurrentWER = (Get-ItemProperty -Path $WerRegPath -Name "Disabled" -ErrorAction SilentlyContinue).Disabled

            if ($CurrentWER -ne $Config.DisableWER) {
                Write-Log "Windows Error Reporting is not set to standard. Enforcing value: $($Config.DisableWER)" "INFO"

                Set-ItemProperty -Path $WerRegPath -Name "Disabled" -Value $Config.DisableWER -Force -ErrorAction Stop

                Write-Log "Windows Error Reporting successfully disabled." "INFO"
            } else {
                Write-Log "Windows Error Reporting is already disabled. No action taken." "INFO"
            }
    } catch {
