...
$ConfigFile  = "C:\Scripts\Config\$ScriptName.json"
...
try {
    ...
    $CurrentVal = (Get-ItemProperty -Path $RegPath -Name "HideFileExt" -ErrorAction SilentlyContinue).HideFileExt

    if ($CurrentVal -ne $Config.HideFileExt) {
        Write-Log "File extensions are hidden. Enforcing visibility." "INFO"
        
        # Enforce registry change and catch errors (e.g., permission issues)
        Set-ItemProperty -Path $RegPath -Name "HideFileExt" -Value $Config.HideFileExt -Force -ErrorAction Stop
        
        Write-Log "File extensions are now visible." "INFO"
    } else {
        Write-Log "File extensions are already visible. No action taken." "INFO"
    }
