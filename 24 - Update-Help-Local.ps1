...
try {
    ...
    try {
        # It's normal for Update-Help to throw non-terminating errors for specific modules that lack online help.
        Update-Help -Force:$Config.Force -ErrorAction $Config.ErrorActionPreference
        Write-Log "PowerShell help files updated successfully (ignoring known non-terminating UI/module errors)." "INFO"
    } catch {
        # Logged as ERROR so the post-flight scanner automatically emails you
        Write-Log "Major failure during Update-Help. $($_.Exception.Message)" "ERROR"
    }
