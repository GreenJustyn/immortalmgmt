...
try {
    ...
    $CurrentRdp = (Get-ItemProperty -Path $RdpRegPath -Name "fDenyTSConnections" -ErrorAction SilentlyContinue).fDenyTSConnections
    $CurrentNla = (Get-ItemProperty -Path $NlaRegPath -Name "UserAuthentication" -ErrorAction SilentlyContinue).UserAuthentication

    $TargetRdp = if ($Config.EnableRDP) { 0 } else { 1 }
    $TargetNla = if ($Config.RequireNLA) { 1 } else { 0 }

    if (($CurrentRdp -ne $TargetRdp) -or ($CurrentNla -ne $TargetNla)) {
        Write-Log "RDP configuration drift detected. Enforcing settings." "INFO"
        
        Set-ItemProperty -Path $RdpRegPath -Name "fDenyTSConnections" -Value $TargetRdp -ErrorAction Stop
        Set-ItemProperty -Path $NlaRegPath -Name "UserAuthentication" -Value $TargetNla -ErrorAction Stop
        
        if ($Config.EnableRDP) {
            Enable-NetFirewallRule -DisplayGroup "Remote Desktop" -ErrorAction SilentlyContinue
            Write-Log "RDP Enabled and Firewall rules activated." "INFO"
        } else {
            Disable-NetFirewallRule -DisplayGroup "Remote Desktop" -ErrorAction SilentlyContinue
            Write-Log "RDP Disabled and Firewall rules deactivated." "INFO"
        }
    } else {
        Write-Log "RDP settings are already correct. No action taken." "INFO"
    }
