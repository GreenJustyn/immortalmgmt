...
try {
    ...
    if (($WinRMService.Status -ne 'Running') -or (-not $Listener)) {
        ...
        Enable-PSRemoting -SkipNetworkProfileCheck:$Config.SkipNetworkProfileCheck -Force:$Config.Force -ErrorAction Stop
        
        Write-Log "PSRemoting enabled and listeners configured." "INFO"
    } else {
        Write-Log "PSRemoting is already enabled and listening. No action taken." "INFO"
    }
