...
try {
    ...
    # Idempotent Logic
    $CurrentPolicy = Get-ExecutionPolicy -Scope $Config.Scope
    if ($CurrentPolicy -ne $Config.DesiredPolicy) {
        Write-Log "Execution policy mismatch. Current: $CurrentPolicy. Enforcing: $($Config.DesiredPolicy)" "INFO"
        
        Set-ExecutionPolicy -ExecutionPolicy $Config.DesiredPolicy -Scope $Config.Scope -Force -ErrorAction Stop
        
        Write-Log "Execution policy updated successfully." "INFO"
    } else {
        Write-Log "Execution policy is already set to $($Config.DesiredPolicy). No action taken." "INFO"
    }
