...
try {
    ...
    # Idempotent Logic
    $SourceExists = [System.Diagnostics.EventLog]::SourceExists($Config.SourceName)

    if (-not $SourceExists) {
        Write-Log "Custom EventLog Source '$($Config.SourceName)' not found. Creating in Log '$($Config.LogName)'." "INFO"
        
        # This requires Administrator privileges
        New-EventLog -LogName $Config.LogName -Source $Config.SourceName -ErrorAction Stop
        
        Write-Log "Custom EventLog Source created successfully." "INFO"
    } else {
        Write-Log "Custom EventLog Source '$($Config.SourceName)' already exists. No action taken." "INFO"
    }
