...
try {
    ...
    # Idempotent Logic
    $ActivePlan = (powercfg /getactivescheme) -match $Config.TargetPlanGUID

    if (-not $ActivePlan) {
        Write-Log "Active power plan is not set to desired GUID. Enforcing now." "INFO"
        
        # Execute native command
        powercfg /setactive $Config.TargetPlanGUID
        
        # Check native exit code to catch silent failures
        if ($LASTEXITCODE -ne 0) {
            Write-Log "Failed to set power plan. powercfg exited with code $LASTEXITCODE." "ERROR"
        } else {
            Write-Log "Power plan successfully set to target ($($Config.TargetPlanGUID))." "INFO"
        }
    } else {
        Write-Log "Power plan is already configured to the target scheme. No action taken." "INFO"
    }
